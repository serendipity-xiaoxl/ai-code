...
const ENTRY_FILE = join(ROOT_DIR, 'src', 'cli', 'index.ts');
...
function parseArgs(): BuildTarget {
  ...
  // Also check for positional args
  for (const a of args) {
...
/**
 * Step 1: Bundle TypeScript to a single JS file using esbuild.
 * @param format - 'esm' (default) or 'cjs' (for Node.js SEA compatibility)
 */
function bundleWithEsbuild(format: 'esm' | 'cjs' = 'esm'): boolean {
  log(`Bundling TypeScript with esbuild (${format})...`);
  ...
  const bundleFile = join(BUNDLE_DIR, `index${ext}`);
  ...
  if (!existsSync(esbuildBin) && !existsSync(esbuildBin.replace('.cmd', ''))) {
    ...
    try {
      execSync(`npx esbuild "${ENTRY_FILE}" --bundle --platform=node --target=node22 --format=${format} --outfile="${bundleFile}" --external:@langchain/* --external:langchain --external:zod --external:commander --external:picocolors`, {
        cwd: ROOT_DIR,
        stdio: 'inherit',
      });
      return true;
  ...
  try {
    execSync(
      `"${esbuildBin}" "${ENTRY_FILE}" --bundle --platform=node --target=node22 --format=${format} --outfile="${bundleFile}" --external:@langchain/* --external:langchain --external:zod --external:commander --external:picocolors`,
      { cwd: ROOT_DIR, stdio: 'inherit' },
    );
    log('Bundle created: ' + bundleFile);
    return true;
  } catch {
    log('ERROR: Failed to bundle with esbuild');
    return false;
...
/**
 * Step 2a: Build standalone binary with Bun.
 */
function buildWithBun(): boolean {
  ...
  try {
    ...
    // Fallback: compile from the bundled JS file
...
/**
 * Step 2b: Build standalone binary with Node.js (pkg).
 */
function buildWithNode(): boolean {
  log('Building standalone binary with Node.js/pkg...');

  // First ensure we have a bundled JS file
  if (!existsSync(BUNDLE_FILE)) {
    if (!bundleWithEsbuild()) return false;
  }
  ...
  if (pkgInstalled) {
    ...
    log('To build with Node.js SEA (Single Executable Application), run:');
    log('  node scripts/build-node-sea.mjs');
  }
...
/**
 * Build with Node.js Single Executable Application (SEA).
 * This is the built-in Node.js way to create binaries (Node 20+).
 */
function buildWithNodeSEA(outPath: string): boolean {
  ...
  try {
    ...
    // Step 3: Inject the SEA blob (requires postject)
...
/**
 * Verify that a binary was built correctly.
 */
function verifyBinary(binPath: string): boolean {
  ...
  try {
    ...
    // Even if --version doesn't work, if the binary runs without error it's OK
    if (result.status === 0) {
...
async function main(): Promise<void> {
  ...
  console.log('');
  console.log(`  ai-code Binary Builder v${APP_VERSION}`);
